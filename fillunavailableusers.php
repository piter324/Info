<?php
	...
	$query = "SELECT * FROM users WHERE loggedin='N' AND username LIKE '%".$_POST[filter]."%' ORDER BY username ASC";
	...
	while($row=mysqli_fetch_array($result))
	{
		if($row[id]!=$_POST[uid])
		{
			...
			echo "<p class='messengerBeltUser' id='".$row[id]."' onclick='startConversation(\"".$row[id]."\")'><span class='inlinowe'><img src='images/ball_unav.png' style='height:12px; margin-bottom:9px;'> </span><span class='inlinowe'><img src='".$profilowe."' style='height:30px; width:30px;'> </span><span class='inlinowe'> ".$identity."</span>";
